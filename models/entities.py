...
class Document:
    def __init__(self, doc_id: int, tokens: List[Token], entities: List[Entity], relations: List[Relation],
        ...
        self._len_encoding = len(encoding)
...
class Dataset:
    ...
    def max_entity_encode_size(self):
        max_size = 0
        for e in self.entities:
            size = e.span_end - e.span_start
            if size > max_size:
                max_size = size
        return max_size

    def max_entity_token_size(self):
        max_size = 0
        for e in self.entities:
            size = len(e.tokens)
            if size > max_size:
    ...
    def max_relation_distance(self):
        max_distance = 0
        for r in self.relations:
            distance = r.head_entity.span_end - r.tail_entity.span_start
